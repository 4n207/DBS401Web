...
public class billDAO {
    ...
    public Bill getBillInfoByBillId(String bill_id) {
        String sql = "select b.bill_id, u.user_email, b.phone, b.address, b.date, b.total, b.payment from bill b inner join users u on b.user_id = u.user_id where b.bill_id = ?";
        Bill bill = null;
        try {
            conn = new DBContext().getConnection();
            ps = conn.prepareStatement(sql);
            ...
            rs = ps.executeQuery();
            if (rs.next()) {
                User user = new User(0, "", rs.getString(2), "", "");
                bill = new Bill(rs.getInt(1), user, rs.getFloat(6), rs.getString(7), rs.getString(4), rs.getDate(5), rs.getInt(3));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
